            ...
            {snippetCategories.map((category) => {
              return (
                <div key={category.id} className="w-[300px] mx-auto">
                  <button
                    className="group relative w-full overflow-hidden border border-border rounded-xl aspect-square transition-all duration-300 dark:hover:border-[#0ea5e9] hover:border-orange-400 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#0ea5e9] bg-background"
                    style={
                      {
                        "--active-color": category.color,
                      } as React.CSSProperties
                    }
                    onClick={() => router.push(`/snippets/${category.id}`)}
                  >
                        ...
                        <div className="flex flex-col gap-1">
                          {category.snippets.map((snippet, idx) => (
                            ...
                            >
                              ...
                              <span className="dark:text-purple-100 text-orange-400 ml-auto">
                                {snippet.tech}
                              </span>
                            </div>
                          ))}
